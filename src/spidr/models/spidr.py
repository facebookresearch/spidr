...
class SpidR(DinoSR):
    ...
    def get_codebooks(self, waveform: Tensor, *, onehot: bool = False) -> list[Tensor | None]:
        x = self.feature_extractor(waveform)
        x = self.feature_projection(x)
        preds: list[Tensor | None] = [None] * (len(self.student.layers) - self.num_codebooks)
        for i, y in enumerate(self.student.get_intermediate_outputs(x)[-self.num_codebooks :]):
