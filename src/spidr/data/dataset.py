...
def crop_audio(
    ...
    *,
    ...
    if length > num_samples and rand_crop:
        frame_offset = int(torch.randint(length - num_samples, size=(1,), generator=generator))
...
class SpeechCollatorWithMasking:
    ...
    def __call__(self, batch: list[Tensor]) -> tuple[Tensor, Tensor, Tensor | None]:
        ...
        wavs_with_len = [
            crop_audio(wav, num_samples, self.max_sample_size, rand_crop=self.rand_crop, generator=self.generator)
            for wav in batch
        ]
        ...
        mask_indices = self.mask_generator(padding_mask, generator=self.generator)[0]
...
def build_dataloader(
    ...
    *,
    ...
    collate_fn = SpeechCollatorWithMasking(
        ...
        mask_and_crop_seed=mask_and_crop_seed,
