...
def make_checkpointable[**P, R](fn: Callable[P, R]) -> Callable[P, R]:
...
def validation_job_config() -> SlurmConfig:
    ...
    return SlurmConfig(account=account, nodes=1, gpus_per_node=1, time=60, cpus_per_task=cpus_per_task)
...
def slurm_config_parse_args(args: argparse.Namespace) -> SlurmConfig:
    ...
    for key in ["time", "cpus_per_task", "mem_per_gpu", "qos", "constraint"]:
